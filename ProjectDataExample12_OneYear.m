...
Covariates = {'Age','Gender','Race','Smoke','BP'};
...
PopColumns1(1) = struct ('Dim','Age','Distribution','','OrderInSet',1);
PopColumns1(end+1) = struct ('Dim','Gender','Distribution','','OrderInSet',2);
PopColumns1(end+1) = struct ('Dim','Race','Distribution','','OrderInSet',3);
PopColumns1(end+1) = struct ('Dim','Smoke','Distribution','','OrderInSet',4);
PopColumns1(end+1) = struct ('Dim','BP','Distribution','','OrderInSet',5);
Pop(1) = struct ('Name', 'Test Distribution Population','Columns', PopColumns1 ,'Data',[54,1,0,0,115 ; 56,1,1,0,125 ;  54,0,0,1,115 ;  56,0,0,0,125] );
...
%
Transitions(end+1) = struct ('StudyID',2,'From', 1, 'To', 2, 'Probability', [] ,'RegCoefVec', [ 0.0112, 1.059, 0.525, 0.39, 1.35, 1.088, 1.078 ] ,'RegParamVec', [1, Age-55, 1-Gender, Race, Smoke, (BP-135.7)/10 , StudyLengthConst] , 'CovarianceMatrix', diag([2.0408e-6, 3.1497e-5, 0.0028699, 0.010412, 0.014994,0.00070387 , 0.00026656]) , 'RegFunctionType', 3  );
